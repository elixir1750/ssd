...
The default path is the standard synchronous DFlash baseline.
The optional [`--all-positions` experiment](docs/dflash-positions.md) prepares
next-round DFlash candidates for every possible accepted length, retaining the
true bonus without enumerating it. It currently runs as a serial reference.
...
Use `--all-positions --draft-async` for the experimental two-GPU SSD execution:
target verification on GPU 0 overlaps branch preparation on GPU 1. See
[the two-GPU API and validation scope](docs/dflash-positions.md#two-gpu-ssd-execution).
